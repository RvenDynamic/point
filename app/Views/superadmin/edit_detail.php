        ...
        <div class="card">
            <div class="card-header">
                <div class="d-flex align-items-center">
                    <h4 class="card-title">Edit Detail Pelanggaran</h4>
                </div>
            </div>
            <div class="card-body">
                <form action="<?= base_url('/superadmin/edit-detail') ?>" method="post" enctype="multipart/form-data">
                    <input type="hidden" value="<?= $point['id_point'] ?>" name="id_point">
                    <!-- Container untuk menambah baris baru -->
                    <div id="additionalRows">
                        <?php foreach ($detail_point as $detail) : ?>
                            <div class="form-group">
                                <select class="form-select" name="id_jenis[]">
                                    <option value="">-- Pilih Jenis Pelanggaran yang dilakukan --</option>
                                    <?php foreach ($jenis as $key => $value): ?>
                                        <option value="<?= $value['id_jenis'] ?>" <?= ($detail['id_jenis'] == $value['id_jenis']) ? 'selected' : '' ?>>
                                            <?= $value['nama_pelanggaran'] ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
